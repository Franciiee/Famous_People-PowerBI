...
('BTS', '2010-06-13', 30, 'Male', 'South Korea', 'Music', 450000000, 2013, 10);
...
-- Youngest Musician(s)
SELECT full_name, primary_industry, age FROM celebrities
WHERE primary_industry = "Music"
...
-- Youngest Actress
SELECT full_name, primary_industry, age FROM celebrities
WHERE primary_industry = "Film"
	AND Age = ( SELECT MIN(AGE)
				FROM celebrities
                WHERE primary_industry = "Film");

-- The Overall Best Celebrity
SELECT c.full_name AS best_celebrity,
	COUNT(a.award_id) AS award_count,
    AVG(co.compensation) AS avg_compensation
FROM Celebrities c
LEFT JOIN Awards a ON c.celebrity_id = a.celebrity_id
LEFT JOIN Contributions co ON c.celebrity_id = co.celebrity_id
GROUP BY best_celebrity
